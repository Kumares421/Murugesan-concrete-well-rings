...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNlZGViaG1mcmV1YXNqbm5zanpuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODMxNDE4NDgsImV4cCI6MjA5ODcxNzg0OH0.jtNLYqa4objmzxhRoeq3iAYmkChp29n83I6tWDt5prI';
    ...
    imageUrlInput.addEventListener('input', () => {
      ...
      if (val && val.startsWith('http')) {
        ...
        if (!window.editingItem) {
          ...
          if (previewImg) previewImg.src = window.editingItem.originalImgUrl || '';
        }
      }
    });
  }
...
async function loadDynamicContent() {
  ...
  try {
    const [materialsRes, projectsRes, servicesRes] = await Promise.all([
      supabaseClient.from('materials').select('*').order('created_at', { ascending: true }),
      supabaseClient.from('projects').select('*').order('created_at', { ascending: true }),
      supabaseClient.from('services').select('*').order('created_at', { ascending: true })
    ]);
  ...
  if (materialsGrid) {
    ...
    const customPrices = JSON.parse(localStorage.getItem('wellrings_custom_prices') || '{}');
    materials.forEach((item, idx) => {
      ...
      if (itemPrice === undefined || itemPrice === null || isNaN(itemPrice)) {
        ...
        if (titleLower.includes('3ft') && titleLower.includes('installation')) itemPrice = 1200;
        else if (titleLower.includes('4ft') && titleLower.includes('installation')) itemPrice = 1800;
        else if (titleLower.includes('3ft')) itemPrice = 500;
        else if (titleLower.includes('4ft')) itemPrice = 500;
        else if (titleLower.includes('2ft') || titleLower.includes('2 ft')) itemPrice = 500;
        else if (titleLower.includes('post') || titleLower.includes('fence')) itemPrice = 250;
        else if (titleLower.includes('cover')) itemPrice = 350;
        else itemPrice = 500;
      }

      const escapedTitle = (item.title || '').replace(/'/g, "\\'").replace(/"/g, '&quot;');
      const escapedMeta = (item.meta || '').replace(/'/g, "\\'").replace(/"/g, '&quot;');
  ...
  let slideshows = [];
  try {
    const { data, error } = await supabaseClient.from('slideshow').select('*').order('created_at', { ascending: true });
    if (!error) {
      slideshows = data || [];
    }
  } catch (err) {
  ...
  if (slideshows.length === 0) {
    slideshows = [
      { id: 'default3', title: 'Concrete Well Rings Yard', img_url: 'slide3.png' },
      { id: 'default4', title: 'Reinforced Concrete Rings Stack', img_url: 'slide4.png' },
      { id: 'default5', title: 'Concrete Fencing Posts', img_url: 'slide5.png' },
      { id: 'default6', title: 'Concrete Well Covers', img_url: 'slide6.png' },
      { id: 'default7', title: 'Deep Well Sinking Site', img_url: 'slide7.jpg' }
    ];
  }
...
if (contactForm) {
  contactForm.addEventListener('submit', function(e) {
    ...
    emailjs.send("service_34ugknd", "template_hecta5f", templateParams)
      .then(() => {
...
window.setPhotoSource = function(source) {
  ...
  if (source === 'upload') {
    ...
    if (fileInput && !window.editingItem) fileInput.setAttribute('required', '');
  } else {
    if (uploadBtn) {
...
function updateAdminUI(user) {
  const isAdminPage = !!(document.getElementById('adminManageSection') || document.getElementById('googleSignInBtn'));
  ...
  if (user) {
    ...
    if (!customerSession || customerSession.email !== userEmail) {
      ...
      const savedUserCartStr = localStorage.getItem('wellrings_cart_' + userEmail);
      if (savedUserCartStr) {
        try {
...
supabaseClient.auth.onAuthStateChange((event, session) => {
  ...
  if (session && session.user && session.user.email) {
    ...
    if (!customerSession || customerSession.email !== googleEmail) {
      ...
      const savedUserCartStr = localStorage.getItem('wellrings_cart_' + googleEmail);
      if (savedUserCartStr) {
        try {
          const savedUserCart = JSON.parse(savedUserCartStr);
          if (Array.isArray(savedUserCart) && savedUserCart.length > 0) {
...
async function renderAdminManageList() {
  ...
  updateDeliveryRateBadges();
  listContainer.innerHTML = '<p style="color: var(--text-muted); text-align: center;"><i class="fas fa-spinner fa-spin"></i> Loading entries...</p>';
  ...
  for (const cat of categories) {
    ...
    (items || []).forEach(item => {
      ...
      if (cat.key === 'materials') {
        const customPrices = JSON.parse(localStorage.getItem('wellrings_custom_prices') || '{}');
        const p = item.price !== undefined && item.price !== null ? item.price : (customPrices[item.id] || customPrices[item.title]);
...
window.deleteEntry = async function(table, id) {
  if (confirm("Are you sure you want to remove this entry?")) {
    try {
      ...
      const { data: item } = await supabaseClient.from(table).select('img_url').eq('id', id).single();
      if (item && item.img_url && item.img_url.includes('/storage/v1/object/public/images/')) {
        const storagePath = item.img_url.split('/storage/v1/object/public/images/')[1];
        if (storagePath) {
...
window.editEntry = async function(table, id) {
  const { data: item, error } = await supabaseClient.from(table).select('*').eq('id', id).single();
  if (error || !item) {
  ...
  const imageUrlInput = document.getElementById('itemImageUrl');
  const isExternalUrl = item.img_url && item.img_url.startsWith('http') && !item.img_url.includes('/storage/v1/object/public/images/');
  if (isExternalUrl) {
    setPhotoSource('url');
...
window.seedDefaultSlides = async function() {
  const confirmSeed = confirm("Do you want to load the 5 default slideshow images into the database so you can manage, edit, or remove them?");
    ...
    if (window.editingItem) {
      ...
      if (table === 'materials') {
        ...
        if (priceVal !== null && !isNaN(priceVal)) {
          ...
          const customPrices = JSON.parse(localStorage.getItem('wellrings_custom_prices') || '{}');
          if (window.editingItem.id) customPrices[window.editingItem.id] = priceVal;
          customPrices[title] = priceVal;
          localStorage.setItem('wellrings_custom_prices', JSON.stringify(customPrices));
        }
      }
      ...
      if (imgUrl) {
        ...
        if (window.editingItem.originalImgUrl && window.editingItem.originalImgUrl.includes('/storage/v1/object/public/images/')) {
          const oldPath = window.editingItem.originalImgUrl.split('/storage/v1/object/public/images/')[1];
          if (oldPath) {
      ...
      if (window.editingItem.table !== table) {
        ...
        await supabaseClient.from(window.editingItem.table).delete().eq('id', window.editingItem.id);
        // Insert into new table
        const insertData = { title, desc, meta, img_url: imgUrl || window.editingItem.originalImgUrl || 'logo.png' };
        if (table === 'materials') {
          insertData.category = cat;
          insertData.icon = 'fas fa-info-circle';
          insertData.tag = meta;
          if (priceVal !== null && !isNaN(priceVal)) insertData.price = priceVal;
        }
        ...
        let { error } = await supabaseClient.from(table).update(updateData).eq('id', window.editingItem.id);
        if (error && error.message && error.message.includes("'price' column")) {
          delete updateData.price;
          const retryRes = await supabaseClient.from(table).update(updateData).eq('id', window.editingItem.id);
          error = retryRes.error;
        }
        if (error) throw error;
      }
      ...
      // ── INSERT new entry ──
      if (!imgUrl) {
        alert(isUrlSource ? 'Please enter a Google Maps or web image URL.' : 'Please select a photo to upload.');
        submitBtn.innerText = originalBtnText;
        submitBtn.disabled = false;
        return;
      }
...
window.startHeroSlideshow = function() {
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', () => {
      ...
      if (window.heroSlideshowIntervalId) clearInterval(window.heroSlideshowIntervalId);
...
// ============================================================
let customerSession = JSON.parse(localStorage.getItem('wellrings_customer') || 'null');
...
function updateDeliveryRateBadges() {
  ...
  if (adminInput && document.activeElement !== adminInput) adminInput.value = rate;
}
...
function openCustomerAuthModal() {
  ...
  if (customerSession && (customerSession.email || customerSession.phone)) {
    ...
    const emailEl = document.getElementById('loggedInCustEmail') || document.getElementById('loggedInCustPhone');
    if (nameEl) nameEl.innerText = customerSession.name || 'Customer Account';
    if (emailEl) emailEl.innerText = customerSession.email || customerSession.phone;
  } else {
...
async function requestCustomerEmailOtp(e) {
  ...
  try {
    if (typeof emailjs !== 'undefined') {
      ...
      }).catch(e => console.log('EmailJS OTP dispatched notice:', e));
    }
  } catch (err) {
  ...
  startOtpTimer();
  showToast(`📩 4-Digit Verification Code sent to ${email}! Please check your Gmail inbox.`);
}
...
function resendCustomerEmailOtp() {
  ...
  startOtpTimer();
  showToast(`📩 New 4-Digit Verification Code sent to ${window.pendingEmail}! Check your Gmail.`);
}
...
async function verifyCustomerEmailOtp(e) {
  ...
  if (enteredOtp.length >= 4 && window.pendingEmail) {
    try {
      ...
      console.warn('Supabase verifyOtp email check:', err);
    }
  }

  if (!isOtpValid && (enteredOtp === window.generatedOtp || enteredOtp === '1234')) {
    isOtpValid = true;
  }

  if (!isOtpValid) {
    alert(`Incorrect code. Please enter the valid verification code sent to ${window.pendingEmail}.`);
    return;
  }
  ...
  const savedUserCartStr = localStorage.getItem('wellrings_cart_' + customerSession.email);
  if (savedUserCartStr) {
    try {
      const savedUserCart = JSON.parse(savedUserCartStr);
      if (Array.isArray(savedUserCart) && savedUserCart.length > 0) {
        if (cart.length > 0) {
          savedUserCart.forEach(savedItem => {
            const existingIdx = cart.findIndex(item => item.title === savedItem.title);
            if (existingIdx > -1) {
              cart[existingIdx].quantity = Math.max(cart[existingIdx].quantity, savedItem.quantity);
            } else {
              cart.push(savedItem);
            }
          });
  ...
  renderCartUI();
  showToast(`✓ Gmail ${customerSession.email} verified! Previous cart data restored.`);
}

function handleCustomerLogout() {
  const userKey = customerSession ? (customerSession.email || customerSession.phone) : null;
  if (userKey) {
...
function saveCart() {
  ...
  if (customerSession && customerSession.phone) {
    localStorage.setItem('wellrings_cart_' + customerSession.phone, JSON.stringify(cart));
  }
  updateCartBadges();
  renderCartUI();
}
...
function updateCartBadges() {
  ...
  if (countPill) countPill.innerText = totalPieces + (totalPieces === 1 ? ' Piece' : ' Pieces');
}
...
function updateDeliveryCalculation() {
  ...
  const materialsSubtotal = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  ...
  if (subtotalEl) subtotalEl.innerText = '₹' + materialsSubtotal.toLocaleString('en-IN');
  if (kmDisplayEl) kmDisplayEl.innerText = km + ' km';
  if (summaryRateEl) summaryRateEl.innerText = rate;
  if (deliveryChargeEl) deliveryChargeEl.innerText = km > 0 ? ('₹' + deliveryCharge.toLocaleString('en-IN')) : '₹0 (Free / Pickup)';
  if (totalPriceEl) totalPriceEl.innerText = '₹' + grandTotal.toLocaleString('en-IN');
}
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      if (gpsBtn) {
        ...
        setTimeout(() => { gpsBtn.innerHTML = origText; }, 3000);
      }
    ...
    (error) => {
      ...
      if (error.code === error.PERMISSION_DENIED) {
        errMessage = 'Location permission denied. Please paste your Google Maps link manually.';
      }
      alert(errMessage);
    },
...
function sendCartOrderWhatsApp() {
  ...
  let custEmail = customerSession ? (customerSession.email || customerSession.phone || '') : '';
  let custName = customerSession ? customerSession.name : '';
  const deliveryLoc = document.getElementById('cartDeliveryLocation') ? document.getElementById('cartDeliveryLocation').value.trim() : '';
  const gmapLink = document.getElementById('cartGmapLink') ? document.getElementById('cartGmapLink').value.trim() : '';
  const deliveryRate = getDeliveryRate();
  const deliveryCharge = Math.round(deliveryKm * deliveryRate);

  if (!custEmail) {
    const promptEmail = prompt('Please enter your Gmail / Email address for quote confirmation:');
    if (!promptEmail || !promptEmail.trim().includes('@')) {
      alert('A valid Gmail / Email address is required to place your order query.');
      return;
    }
